...
export default function Messages() {
  ...
  return (
    <div className="relative w-full max-w-6xl h-[82vh] flex gap-4 md:gap-6 items-stretch justify-center mx-auto mt-2 sm:mt-6 px-2 sm:px-4">
      
      {/* Sidebar - Inbox */}
      <div className={`w-full md:w-[350px] h-full glass rounded-[30px] sm:rounded-[40px] flex-col overflow-hidden shadow-[0_20px_50px_rgba(168,85,247,0.15)] relative animate-in fade-in slide-in-from-left-8 duration-500 ${activeConvId ? 'hidden md:flex' : 'flex'}`}>
      ...
      {/* Main Chat Thread */}
      <div className={`w-full md:flex-1 h-full glass rounded-[30px] sm:rounded-[40px] flex-col overflow-hidden shadow-[0_20px_50px_rgba(168,85,247,0.15)] relative bg-white/40 animate-in fade-in slide-in-from-right-8 duration-500 ${!activeConvId ? 'hidden md:flex' : 'flex'}`}>
        {activeConv ? (
          <>
            {/* Thread Header */}
            <div className="h-[75px] sm:h-[90px] bg-white/80 backdrop-blur-xl px-4 sm:px-10 flex justify-between items-center border-b border-primary/10 shadow-sm z-10">
              <div className="flex items-center gap-3 sm:gap-5">
                {/* Back Button for mobile */}
                <button 
                  onClick={() => setActiveConvId(null)}
                  className="md:hidden p-1.5 -ml-1 rounded-full hover:bg-primary/10 text-primary border-none bg-transparent cursor-pointer flex items-center justify-center"
                >
                  <ChevronLeft size={24} />
                </button>

                <div className="relative cursor-pointer group">
                  <img src={activeConv.user.avatar} alt="avatar" className="w-[42px] h-[42px] sm:w-[52px] sm:h-[52px] rounded-full object-cover shadow-sm group-hover:scale-105 transition-transform" />
                  <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-[2px] border-white" />
                </div>
                <div>
            ...
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-4 sm:p-10 flex flex-col gap-4 sm:gap-6 relative bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-opacity-10">
            ...
            {/* Input Area */}
            <div className="p-4 sm:p-6 bg-white/90 backdrop-blur-xl border-t border-primary/10 z-10 shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
              <div className="flex items-center gap-2 sm:gap-3 bg-gray-50 p-2 sm:p-2.5 rounded-full border border-primary/20 shadow-inner group focus-within:border-primary focus-within:bg-white transition-all">
                <input 
                  type="text" 
                  value={inputText}
