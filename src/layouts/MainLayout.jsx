...
const SidebarLink = ({ to, icon: Icon, label, isCollapsed, onClick }) => (
  <NavLink
    ...
    className={({ isActive }) =>
      `flex items-center ${isCollapsed ? 'justify-center px-0' : 'gap-4 px-4'} py-3.5 rounded-2xl font-extrabold text-sm transition-all duration-300 border border-transparent cursor-pointer ${isActive
...
const MobileNavLink = ({ to, icon: Icon, label, onClick }) => (
  <NavLink
    ...
    className={({ isActive }) =>
      `flex flex-col items-center justify-center w-12 sm:w-14 h-12 rounded-xl transition-all duration-300 ${isActive
  ...
  return (
    <div className="flex w-screen h-screen bg-[#fafafa] text-primary font-inter overflow-hidden relative">
      ...
      <div className="flex-1 h-screen flex flex-col relative overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden bg-[#fafafa] p-4 md:p-8 pb-[80px] lg:pb-8">
          <Outlet />
        </main>
